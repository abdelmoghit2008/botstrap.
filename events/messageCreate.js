...
*/
    ...
    if (neededPerms.length) {
      ...
      .setColor('#ff0000')
      .setDescription(`❌ ** I don't have Premission - [${neededPerms}]**`)
  ...
  if(command.authorPermission) {
    ...
    if (neededPerms.length){
      ...
      .setColor('#ff0000')
      .setDescription(`❌ ** You don't have Premission - [${neededPerms}]**`)
...
if(!message.channel.type === "dm") return ;
  ...
  if (!timestamps.has(member.id)) {
    ...
    if (now < expirationTime) {
      ...
      .setDescription(`**Please Wite CoolDown ${timeLeft.toFixed(0)} Seconds**!`)
      .setAuthor({name: message.author.tag,iconURL:
        message.author.avatarURL({dynamic:true})})
      return message.channel.send({ embeds: [timeembed] }).then(timeembed => {
      setTimeout(() => timeembed.delete(), 3000) && message.delete();
